# SilHomeWorksBot by Goose studios
# Обратная связь - https://forms.yandex.ru/u/6537ef57505690dfcdbf9ca3/
...
clases = ["5с", "6с", "7с", "7т", "8с", "8т", "9с", "10с", "11с", "13е"]
weatherAPIkey = "061d031e50b24373be834b226c3d75a7"
get_weather_URL = f"https://api.openweathermap.org/data/2.5/weather?q=Moscow&appid={weatherAPIkey}&units=metric"
...
cur.execute("CREATE TABLE IF NOT EXISTS homeworks (class varchar(8), teach varchar(64), is_act varchar(8), writer varchar(64), text varchar(64), lesson varchar(64), data varchar(64))")
conn.commit()
...
rasp = {
    ...
    1: {
        ...
        9: [[1, "Математика (Заводов АА) 209"], 
            ...
            [7, "Физика (Репнев) 402"], [8, "Физика (Репнев) 402"], 
    ...
    4: {
        ...
        8: [[1, "ДОП Олимп.экономика (Кирюшин) 202 / Англ (Домбровская) 405	Англ (Слоневская) 404"], 
            ...
            [3, "Театр 508 / Физика (Репнев) 402"], [4, "Театр 508 / Физика (Репнев) 402"], 
...
def update_childrens():
    ...
    childrens.append(list(('13Е', 'Гусев', 'Антон', 'Алексеевич')))
    # print(childrens)
    ...
    #         if str(i[1]).strip() == f'{childrens[j][1]} {childrens[j][2]} {childrens[j][3]}'.strip():
    #            childrens.remove(childrens[j])
    # print(childrens)
    # cur.close()
    # conn.commit()
    # conn.close()
...
def new_ticket(message, text, name):
    id = 1
    ...
    cur = conn.cursor()
    cur.execute("INSERT INTO tickets (id, user_id, name, text, level) VALUES ('%s', '%s', '%s', '%s', 0)" % (id, message.from_user.id, name, text))
    conn.commit()
    print(id)
    cur.close()
    conn.close()

def new_hw(message, date, clas, teach, writer, text, lesson):
    cur = conn.cursor()
    cur.execute("INSERT INTO homeworks (class, teach, is_act, writer, text, lesson, data) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s')" % (clas, teach, 'True', writer, text, lesson, date))
    conn.commit()
    print(id)
    cur.close()
    conn.close()
...
def main(message):
    ...
    elif is_notacceptfamily(message):
        bot.send_message(message.chat.id, f"Здравствуйте, {message.from_user.first_name}! Вы зарегистрированы как родитель, но ученик должен это подтвердить.")
...
def callback_message(callback):
    if callback.data == "info_about_reg":
        bot.send_message(
            ...
            f"Ученик: может просматривать и изменять задание для своего класса на 2 недели вперёд. (для регистрации нужно имя и класс)\n\nУчитель может просматривать и изменять домашнее задание у всех классов (для регистрации нужно написать админу(ам) ({admins}))\n\nРодственник: может просматривать домашнее задание у класса его ученика (для регестрации требуется подтверждение с аккаунта ученика)",
        )
    elif callback.data == "student":
        register(callback.message)
    elif callback.data == "teacher":
        bot.send_message(
            callback.message.chat.id, f"Пропишите /regt, но пока это не работает"
        )
    elif callback.data == "family":
        bot.send_message(
            callback.message.chat.id, f"Пропишите /regf, но пока это не работает"
        )
    elif callback.data == "yes_registred_me":
        bot.send_message(callback.message.chat.id, f"Хорошо, значит вы готовы продолжать. Пожалуйста, введите свой класс (регистр не важен). Например 6с, 11с")
...
def register(message):
    if not (is_reg(message)):
        ...
        bot.send_message(message.chat.id, f"Вы хотите зарегестрироватся как ученик?", reply_markup=markup)
    else:
...
def teacher_class(message):
    ...
    conn.close()
    bot.send_message(
        message.chat.id, f"Мы зарегистрировали учителя, и написали ему об этом."
    )
...
def reg(message):
    ...
    bot.send_message(
        message.chat.id,
        f"Прекрастно! Учитель зарегистрирован! Вы можете проверить это через /allr",
    )
...
def main(message):
    if not (is_reg(message)):
        bot.send_message(
            message.chat.id,
            f"Вы хотите зарегистрировался как родитель? Введите Y или y если хотите и N или n если нет.",
        )
        bot.register_next_step_handler(message, Ynf)
    else:
        bot.send_message(message.chat.id, f"Вы уже зарегестрированны!")
...
def Ynf(message):
    ...
    if text == "y":
        bot.send_message(
            message.chat.id,
            f"Хорошо, значит вы готовы продолжать. Теперь введите своё ФИО",
        )
        bot.register_next_step_handler(message, fiofamily)
    elif text == "n":
        bot.send_message(
            message.chat.id,
            f"Хорошо, тогда снова пропишите /start и выбирете за кого вы хотите зарегистрироватся",
        )
    else:
        bot.send_message(
            message.chat.id,
            f"Сочтём это за нет. Снова пропишите /start и выбирете за кого вы хотите зарегистрироватся",
        )

def fiofamily(message):
    ...
    while True:
        ...
        cur.execute("SELECT * FROM family_codes")
        fcodes = cur.fetchall()
        # print(users)
        tf = False
        for i in fcodes:
            if code == message.from_user.id:
                tf = True
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO users (id, name, type, class, mut, admin) VALUES ('%s', '%s', 'notacceptfamily', 'none', 'false', 'false')"
        % (message.from_user.id, fio, cl)
    )
    conn.commit()
    cur.execute(
        "INSERT INTO family_codes (code, user_id) VALUES ('%s', '%s')"
    ...
    bot.send_message(
        message.chat.id,
        f"Сейчас мы выдадим вам специальный код. Вот он: {code}. Сообщите этот код ученику к аккаунту которого вы хотите привязать свой аккаунт. Он должен ввести команду /addfamily, и ввести этот код туда",
    )

@bot.message_handler(commands=["addfamily"])
def main(message):
    if is_reg(message):
        if is_student(message):
            bot.send_message(message.chat.id, f"Вы не ученик!")
        else:
            bot.send_message(message.chat.id, f"Вы хотите добавить родителя? (Y/n)")
            bot.register_next_step_handler(message, Ynaf)
    else:
        bot.send_message(message.chat.id, f"Вы не зарегистрированы!")
...
def Ynaf(message):
    ...
    if text == "y":
        bot.send_message(
            message.chat.id,
            f"Хорошо, значит вы готовы продолжать. Теперь введите код совего родителя",
        )
        bot.register_next_step_handler(message, addfamcode)
    elif text == "n":
        bot.send_message(
            message.chat.id,
            f"Хорошо, тогда снова пропишите /start и выбирете за кого вы хотите зарегистрироватся",
...
def addfamcode(message):
    ...
    bot.send_message(
        message.chat.id,
        f"Хорошо. Если вы через несколько секунд не получите следующего сообщения, обратитесь в поддержку",
    )

    conn = sqlite3.connect("test.gus")
    cur = conn.cursor()

    cur.execute("UPDATE users SET type = 'family' WHERE id = '%s'" % (id))
    conn.commit()

    cur.execute(
        "UPDATE users SET class = '%s' WHERE id = '%s'" % (message.from_user.id, id)
    )
    conn.commit()

    cur.execute("DELETE from family_codes WHERE code = '%s'" % (code))
    conn.commit()

    cur.close()
    conn.close()

    bot.send_message(message.chat.id, f"Родитель зарегистрирован!")
...
def main(message):
    goals1 = [
        ["Сделать регистрацию для детей", 2],
        ["Сделать регистрацию для учителей", 1],
        ["Сделать регистрацию для родителей", 0],
        ["Сделать логи", 2],
        ["Сделать домашнее задание", 0],
        ["Сделать погоду", 2],
        ["Сделать тикеты", 0.5],
        ["Сделать инвентарь для значков", 0],
        ["Сделать сообщения", 0],
        ["Приправить пасхалочками", 1],
        ["Исправить граматическые ошибки", 0],
        ["Добится популярности", 0],
    ]
    moregoals = [
        ["Добавить возможность отмечать свой прогресс в домашнем задании", 0],
    ]
    outp = "Планы на обновление 1.0\n"
    ...
    outp += "\nПланы на будующие обновления:\n"
    for i in range(len(moregoals)):
        outp += moregoals[i][0]
        outp += ": "
        if moregoals[i][1] == 0:
            outp += "🔴"
        elif moregoals[i][1] == 1:
            outp += "🟡"
        elif moregoals[i][1] == 2:
            outp += "🟢"
        else:
            outp += '❓❓❓'
        outp += "\n"
...
def main(message):
    if is_reg(message):
        ...
        # outp += '/raspnw - расписание на следующую неделю\n'
        bot.send_message(
            message.chat.id,
        ...
        bot.send_message(
            message.chat.id,
            f"Здравствуйте, {message.from_user.first_name}! К сожалению вы не зарегистрированны. Вам нужно сделать это прописав /start",
        )
...
def main(message):
    if is_student(message) or is_teacher(message):
        if message.text.strip() == '/ehw' or '/edithomework':
            bot.send_message(message.chat.id, f'После {message.text.strip()} введите дату на которое вы хотите изменить дз')
        else:
            ehwdate = message.text.strip().split()[1:3]
            global sethwlist
            sethwlist = []
            if is_student():
                outp = 'На какой урок вы хотите изменить домашнее задание?'
                for i in rasp[take_weekday(ehwdate[0], ehwdate[1], ehwdate[2])][user_clas(message)]:
                    sethwlist.append(i)
            elif is_teacher():
...
@bot.message_handler(commands=["nwhw", "nextweekhomework"])
def main(message):
    bot.send_message(message.chat.id, f"Кажется пока это не работает")
...
def main(message):
    ...
    if bet_weather:
        try:
            weather = requests.get(get_weather_URL)
            if weather.status_code == 200:
                bot.send_message(message.chat.id, f"Мы начали поиск погоды. Если вы долго не получаете ответ, значчит погода сломалась")
                weather = requests.get(get_weather_URL)
                if weather.status_code == 200:
                    ...
                    if main_0 == "Clouds":
                        ...
                        main_0 = "Ясно"
                    bot.reply_to(message, f'Сейчас погода в Москве:\nТемпаратура: {int(data["main"]["temp"])}°C (ощущается как {data["main"]["feels_like"]}°C)\n{main_0}',)
            else:
...
@bot.message_handler(commands=["tickets", "tick"])
def main(message):
    bot.send_message(
        message.from_user.id,
        f"Создать тикет: /cretick\nОтвечать на тикеты (для модераторов и выше): /answtick\nПоследние тикеты (для создателей): /alltick",
    )

@bot.message_handler(commands=["cretick"])
def main(message):
    if is_reg(message):
        bot.send_message(message.from_user.id, f"Хорошо. Введите название (тему) тикета. если вы передумали создавать тикет введите 'cansel'")
        bot.register_next_step_handler(message, cretickname)
    else:
        bot.send_message(message.from_user.id, f"Вы не можете создавать тикеты так как вы не зарегистрированны")

def cretickname(message):
    global fromcretickname
    fromcretickname = message.text.strip().lower()
    if fromcretickname == 'cansel':
        bot.send_message(message.from_user.id, f"Хорошо, создание тикета отменено")
    else:
        bot.send_message(message.from_user.id, f'Хорошо. Введите описание тикета')
        bot.register_next_step_handler(message, cretickopis)

def cretickopis(message):
    global fromcretickopis
    fromcretickopis = message.text.strip().lower()
    new_ticket(message, fromcretickname, fromcretickopis)

@bot.message_handler(commands=["answtick"])
def main(message):
    bot.send_message(message.from_user.id, f"Кажется это пока не работает")

@bot.message_handler(commands=["alltick"])
def main(message):
    bot.send_message(message.from_user.id, f"Кажется это пока не работает")
